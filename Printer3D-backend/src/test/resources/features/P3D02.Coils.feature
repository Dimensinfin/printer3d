...
Feature: Manage the Coils on the Inventory information section
  ...
  Scenario: [P3D02.05] Validate that not all the new Coil fields should be set up to have a valid coil. There are default values.
    ...
    And the Coil repository has a record for new Coil with the next fields
      | id         | material | tradeMark | color  | label  | weight | active |
      | <world:id> | PLA      | ESUN      | BLANCO | BLANCO | 750    | true   |
  ...
  Scenario: [P3D02.06] The Color Set can have a different value from the color label.
    ...
    And the Coil repository has a record for new Coil with the next fields
      | id         | material | tradeMark | color       | label  | weight | active |
      | <world:id> | PLA      | ESUN      | BLANCO ROTO | BLANCO | 750    | true   |

  @P3D02.H @P3D02.07
  Scenario: [P3D02.07] The color label for a Coil can now be changed. Validate that this is possible. Weight is always mandatory.
    Given a clean Coil repository table
    And the following Coils in my service
      | id                                   | material | tradeMark | color    | label    | weight | active |
      | 3bcb9a1f-fa2e-42a9-8a66-d05a7453a61e | TPU      | ESUN      | ROJO     | ROJO     | 500    | true   |
    ...
    And the coil with id "49f72da1-051c-437c-b9d1-b81e298b156d" of the list of Coils has the next fields
      | id                                   | material | tradeMark | color | label | weight | active |
      | 49f72da1-051c-437c-b9d1-b81e298b156d | TPU      | ESUN      | GRIS  | NEGRO | 500    | true   |

  @P3D02.H @P3D02.08
  Scenario: [P3D02.08] The active flag for a Coil can now be changed. Validate that this is possible. Weight is always mandatory.
    Given a clean Coil repository table
    And the following Coils in my service
      | id                                   | material | tradeMark | color    | label    | weight | active |
      | 3bcb9a1f-fa2e-42a9-8a66-d05a7453a61e | TPU      | ESUN      | ROJO     | ROJO     | 500    | true   |
  ...
  @P3D02.H @P3D02.09
  Scenario: [P3D02.09] The Coil another new field that is not mandatory when updating a Coil.
    Given a clean Coil repository table
    And the following Coils in my service
      | id                                   | material | tradeMark | color    | label    | weight | active |
      | 3bcb9a1f-fa2e-42a9-8a66-d05a7453a61e | TPU      | ESUN      | ROJO     | ROJO     | 500    | true   |
...
#  @P3D01.E @P3D01.E.01
#  Scenario: [P3D01.E.01]
#  [P3D02.05] If any of the mandatory Coil fields is not set on the request then we obtain and exception
