// - DOMAIN
import { RequestForm } from './RequestForm.dto';
...
describe('CLASS RequestForm [Module: DTO]', () => {
    ...
    describe('Construction Phase', () => {
        it('constructor.empty: validate initial state without constructor', () => {
            ...
            expect(instance.partsToServe).toBeDefined();
            expect(instance.partsToServe.length).toBe(0);
        });
    ...
    describe('Code Coverage Phase [Methods]', () => {
        it('addPart: add a part to the Request', async () => {
            ...
            expect(instance.partsToServe.length).toBe(1);
        });
        it('removePart: remove a part from the Request', async () => {
            expect(instance.partsToServe).toBeDefined();
            expect(instance.partsToServe.length).toBe(0, 'Number of Parts at creation');
            const part1: Part = new Part({ labe: 'Part1' })
            const part2: Part = new Part({ labe: 'Part2' })
            instance.addPart(part1);
            instance.addPart(part1);
            instance.addPart(part2);
            expect(instance.partsToServe.length).toBe(3, 'Number of parts after all additions.');
            instance.removePart(part1)
            expect(instance.partsToServe.length).toBe(1, 'Number of parts after removal');
        });
    });
});
