...
if "%APIRECORDER_LOG_PATH%"=="" set "APIRECORDER_LOG_PATH=%APIRECORDER_HOME%\logs"
if not exist "%APIRECORDER_LOG_PATH%" (
  @echo Simulation logs directory "%APIRECORDER_LOG_PATH%" doesn't exist
  goto End
)

if "%APIRECORDER_LOG_NAME%"=="" set "APIRECORDER_LOG_NAME=apirecorder.log"
if "%APIRECORDER_OUT_NAME%"=="" set "APIRECORDER_OUT_NAME=apirecorder.out"

if ""%1"" == ""stop"" (
  set "APIRECORDER_LOG_NAME=apirecorder-stop.log"
  set "APIRECORDER_OUT_NAME=apirecorder-stop.out"
)

set "APIRECORDER_LOG_FILE_SPEC=%APIRECORDER_LOG_PATH%\%APIRECORDER_LOG_NAME%"
set "APIRECORDER_OUT_FILE_SPEC=%APIRECORDER_LOG_PATH%\%APIRECORDER_OUT_NAME%"
...
set "APIRECORDER_LOG_ARGS=%APIRECORDER_LOG_ARGS% -Dapirecorder.log.fileSpec=%APIRECORDER_LOG_FILE_SPEC% "
...
if "%APIRECORDER_HEAP_SIZE%"=="" set "APIRECORDER_HEAP_SIZE=-Xms128m -Xmx128m"
set "APIRECORDER_OPTS=-server %APIRECORDER_OPTS% %APIRECORDER_HEAP_SIZE%"

if "%TITLE%" == "" set "TITLE=API Recorder"
@rem start "%TITLE%" /min cmd /c ""%APIRECORDER_JAVA%\bin\java" %APIRECORDER_OPTS% -cp "%CP%" %APIRECORDER_LOG_ARGS% com.apisimulator.http.recorder.APIRecorder %CMD_LINE_ARGS% > %APIRECORDER_OUT_FILE_SPEC% 2>&1" 
...
"%APIRECORDER_JAVA%\bin\java" %APIRECORDER_OPTS% -cp "%CP%" %APIRECORDER_LOG_ARGS% com.apisimulator.http.recorder.APIRecorder %CMD_LINE_ARGS% 2>&1
goto End
...
@rem "%APIRECORDER_JAVA%\bin\java" %APIRECORDER_OPTS% -cp "%CP%" %APIRECORDER_LOG_ARGS% com.apisimulator.http.recorder.APIRecorder -stop %CMD_LINE_ARGS% > %APIRECORDER_OUT_FILE_SPEC% 2>&1
