...
if ""%1"" == ""start"" (
  if "%APISIMULATOR_HEAP_SIZE%"=="" set "APISIMULATOR_HEAP_SIZE=-Xms256m -Xmx256m"
...
@rem After the intro of single simlets config file, the directory may not exist
@rem if not exist "%APISIMULATION_SIMLETS_DIR%" (
@rem   @echo Simulation simlets directory "%APISIMULATION_SIMLETS_DIR%" doesn't exist
@rem   goto End
@rem )

if "%APISIMULATION_LOG_PATH%"=="" set "APISIMULATION_LOG_PATH=%APISIMULATION_HOME%\logs"
if not exist "%APISIMULATION_LOG_PATH%" (
  set "APISIMULATION_LOG_PATH=%APISIMULATION_HOME%"
)
set "APISIMULATION_OUT=%APISIMULATION_LOG_PATH%\apisimulator.out"
...
@rem start "%TITLE%" /min cmd /c ""%APISIMULATOR_JAVA%\bin\java" %APISIMULATOR_OPTS% -cp "%CP%" %APISIMULATION_LOG_ARGS% com.apisimulator.APISimulator -c "%APISIMULATOR_CONFIG_DIRS%" -s "%APISIMULATION_SIMLETS_DIR%" %CMD_LINE_ARGS% > %APISIMULATION_OUT% 2>&1"
@rem start "%TITLE%" /min cmd /c ""%APISIMULATOR_JAVA%\bin\java" %APISIMULATOR_OPTS% -cp "%CP%" %APISIMULATION_LOG_ARGS% com.apisimulator.APISimulator -c "%APISIMULATOR_CONFIG_DIRS%" -s "%APISIMULATION_SIMLETS_DIR%" %CMD_LINE_ARGS% 2>&1"
     start "%TITLE%" /min cmd /c ""%APISIMULATOR_JAVA%\bin\java" %APISIMULATOR_OPTS% -cp "%CP%" %APISIMULATION_LOG_ARGS% com.apisimulator.APISimulator -c "%APISIMULATOR_CONFIG_DIRS%" -s "%APISIMULATION_SIMLETS_DIR%" %CMD_LINE_ARGS%"
goto End
