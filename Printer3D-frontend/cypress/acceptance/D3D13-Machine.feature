...
Feature: [D3D13]-[STORY] Steps to define the interactions with a Machine.
    ...
    Scenario: [D3D13.03]-When one Part is dropped on the Part slot then the right panel activates with the time counter and the interaction buttons.
        ...
        And the target item button with name "COMENZAR" has a label "Comenzar" and is "enabled"
    ...
    @D3D13 @D3D13.05
    Scenario: [D3D13.05]-If the Start button is clicked then the Machine changes to RUNNING state, the josbis submited for build to the backend and the timer starts to countdown.
