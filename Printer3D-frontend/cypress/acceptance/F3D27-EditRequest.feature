@F3D27
Feature: [F3D27]-Check the acceptance for the Customer Request Edit page and actions.

    [STORY] There is a new Customer Request Edit page that allows to change the contents of an existing Request that is not already delivered.

    Background: Application landing page
        Given the application Printer3DManager
        # Given the Feature "/PEDIDOS" is hovered
        # When there is a click on Feature "/PEDIDOS CERRADOS"
        Then the page "Open Request Edit Page" is open
        When the application completes loading

    # - H A P P Y   P A T H
    @F3D26.01
    Scenario: [F3D26.01]-Validate the page structure. Two sections and the panel titles.
        Then the page "Open Request Edit Page" has 2 sections
        Given the target is the panel of type "available-request-elements"
        Then the target has the title "/STOCKS DISPONIBLES"
        Given the target is the panel of type "edit-request"
        Then the target has the title "/PEDIDO/EDICION/DETALLE"
