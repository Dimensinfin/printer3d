...
Feature: [D3D18]-[STORY] Parts have fields editable both at the generic definition level and at the instance definition level
    ...
    #     Then the target item field named "description" is editable
    #     Then the target item field named "buildTime" is editable
    #     Then the target item field named "weight" is editable
    #     Then the target item field named "imagePath" is editable
    #     Then the target item field named "modelPath" is editable
    #     Then the target item has a actionable image named "update-button"
    #     And the target panel has an input field named "description" with label "DESCRIPCION" and contents "{string}"
    #     And the target panel has an input field named "buildTime" with label "TIEMPO" and contents "{string}"
    #     And the target panel has an input field named "weight" with label "PESO" and contents "{string}"
    #     And the target panel has an input field named "imagePath" with label "IMAGEN" and contents "{string}"
    #     And the target panel has an input field named "modelPath" with label "FICHERO IMPR." and contents "{string}"
    ...
    Scenario: [D3D18.03]-Validate the contents of a Part Container. Now there are new fields and the Image and Model File should also be visible if not empty.
        ...
        And the target item has a field named "buildTime" with label "TIEMPO" and value "0 min."
