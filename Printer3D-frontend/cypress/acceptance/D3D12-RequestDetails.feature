...
Feature: [D3D12]-[STORY] When a request is selected then we can see the details.
    ...
    Scenario: [D3D12.08]-If the Delete button of the Confirmation dialog is clicked then the Request is deleted, a notification shown and the dialog closes.
        ...
        Then the Request is updated on the backend
        And there is a Notification panel
        # WARNING: Thre is some problem on the next actions
        # And the dialog closes

    # @D3D12 @D3D12.09
    # Scenario: [D3D12.09]-After the Request is deleted and the dialog closes the Open Requests page should refresh and the number of Requests opens should be smaller.
    # WARNING: This is not possible with the current api simulation because the returned data from the backend is always the same.
