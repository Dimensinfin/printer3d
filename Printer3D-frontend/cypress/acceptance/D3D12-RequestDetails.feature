...
Feature: [D3D12]-[STORY] When a request is selected then we can see the details.
    ...
    @D3D12.06
    Scenario: [D3D12.06.02]-The Request detail for an OPEN request always shows a Delete button. If this button is clicked there there is a confirmation dialog.
        # - Select a request to check its details.
        Given the target is the panel of type "open-requests"
        Given the target the "request" with id "88d5785a-6bb4-4d89-9b2b-f590a1112d31"
        Given the target is clicked
    ...
    @D3D12.07
    Scenario: [D3D12.07]-When deleting a request if the CANCEL button of the Confirmation dialog is clicked then the request dialog closes.
        # - Select a request to check its details.
        Given the target is the panel of type "open-requests"
        Given the target the "request" with id "a00f7e7a-56c4-4dc1-a630-2b2a62b54eb9"
        Given the target is clicked
    ...
    @D3D12 @D3D12.08
    Scenario: [D3D12.08]-When deleting a request if the DELETE button of the Confirmation dialog is clicked then the Request is deleted, a notification shown and the dialog closes.
        # - Select a request to check its details.
        Given the target is the panel of type "open-requests"
    ...
    @D3D12 @D3D12.E.02
    Scenario: [D3D12.E.02]-When deleting a request if the Request is already delivered we notify an exception and do not close the dialog.
        # - Select a request to check its details.
        Given the target is the panel of type "open-requests"
        Given the target the "request" with id "a00f7e7a-56c4-4dc1-a630-2b2a62b54eb9"
        Given the target is clicked
        ...
        Then the "Delete Confirmation" dialog opens and blocks the display
        # - Change the api simulator behavior
        Given response "409-CONFLICT" for "Get Open Requests"
        # - Click on the Delete of the Confirmation dialog
        Given the target is the panel of name "delete-request-dialog"
        And the button with name "delete-button" has a label "Borrar" and is "enabled"
        When the button with name "delete-button" is clicked
